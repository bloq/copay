...
angular.module('copayApp.controllers').controller('tabSendController', function($scope, $rootScope, $log, $timeout, $ionicScrollDelegate, addressbookService, profileService, lodash, $state, walletService, incomingData, popupService, platformInfo, bwcError, gettextCatalog) {
  ...
  var updateWalletsList = function() {
    ...
    $scope.showTransferCard = $scope.hasWallets && lodash.some(networkResult, function (count) {
      return count > 1;
    });

    if ($scope.showTransferCard) {
      var walletsToTransfer = $scope.wallets;
      Object.keys(networkResult).forEach(function (network) {
        if (networkResult[network] < 2) {
          walletsToTransfer = walletsToTransfer.filter(function (item) {
